...
# --- COMPLETE HTML BLUEPRINT (UNCHANGED) ---
HTML_FRAMEWORK = """
<!DOCTYPE html>
<html>
<head>
...
if 'selected_ta' not in st.session_state:
    st.session_state.selected_ta = "Select Trade Area..."
if 'selected_site' not in st.session_state:
    st.session_state.selected_site = "Select Site..."

# --- BUILD THE MINIMAL UI WITH APPROACH 3 ---
def process_val(site_row_data, key_string):
    val = site_row_data.get(key_string.upper(), "")
    if pd.isna(val) or val is None: return ""
    return str(val).strip()

def build_full_ui(selected_ta, selected_site, site_row_data=None, valid_photos=None, valid_docs=None):
    ...
    if selected_site and selected_site != "Select Site...":
        # Build the rendered HTML framework with data
        rendered_view = HTML_FRAMEWORK
        rendered_view = rendered_view.replace("_TRADE_AREA_", process_val(site_row_data, "TRADE AREA"))
        rendered_view = rendered_view.replace("_SITE_NAME_", process_val(site_row_data, "SITE NAME"))
        rendered_view = rendered_view.replace("_SITE_NO_", process_val(site_row_data, "SITE NO"))
        rendered_view = rendered_view.replace("_TIMESTAMP_", process_val(site_row_data, "TIMESTAMP"))
        rendered_view = rendered_view.replace("_DATE_OF_REPORT_", process_val(site_row_data, "DATE OF REPORT"))
        rendered_view = rendered_view.replace("_UNIT_BLDG_ST_NAME_", process_val(site_row_data, "UNIT #, BLDG/ST # AND ST NAME"))
        rendered_view = rendered_view.replace("_BARANGAY_DISTRICT_NAME_", process_val(site_row_data, "BARANGAY/DISTRICT NAME"))
        rendered_view = rendered_view.replace("_CITY_MUNICIPALITY_", process_val(site_row_data, "CITY/MUNICIPALITY"))
        rendered_view = rendered_view.replace("_REGION_", process_val(site_row_data, "REGION"))
        rendered_view = rendered_view.replace("_POSTAL_CODE_", process_val(site_row_data, "POSTAL CODE"))
        rendered_view = rendered_view.replace("_SITE_AVAILABILITY_DATE_", process_val(site_row_data, "SITE AVAILABILITY DATE"))
        rendered_view = rendered_view.replace("_MONTHLY_RENTAL_RATE_", process_val(site_row_data, "MONTHLY RENTAL RATE"))
        rendered_view = rendered_view.replace("_COL_START_DATE_", process_val(site_row_data, "COL START DATE"))
        rendered_view = rendered_view.replace("_COL_END_DATE_", process_val(site_row_data, "COL END DATE"))
        rendered_view = rendered_view.replace("_LEASE_TERMS_", process_val(site_row_data, "LEASE TERMS"))
        rendered_view = rendered_view.replace("_ESCALATION_", process_val(site_row_data, "ESCALATION"))
        rendered_view = rendered_view.replace("_ADVANCE_RENTAL_", process_val(site_row_data, "ADVANCE RENTAL"))
        rendered_view = rendered_view.replace("_SECURITY_DEPOSIT_", process_val(site_row_data, "SECURITY DEPOSIT"))
        rendered_view = rendered_view.replace("_CUSA_", process_val(site_row_data, "CUSA"))
        rendered_view = rendered_view.replace("_LOT_FLOOR_AREA_SQM_", process_val(site_row_data, "LOT/FLOOR AREA SQM"))
        rendered_view = rendered_view.replace("_FRONTAGE_", process_val(site_row_data, "FRONTAGE"))
        rendered_view = rendered_view.replace("_LEASE_TYPE_", process_val(site_row_data, "LEASE TYPE"))
        rendered_view = rendered_view.replace("_LESSOR_", process_val(site_row_data, "LESSOR"))
        rendered_view = rendered_view.replace("_CONTACT_PERSON_SOURCE_", process_val(site_row_data, "CONTACT PERSON/SOURCE"))
        rendered_view = rendered_view.replace("_CONTACT_NUMBER_", process_val(site_row_data, "CONTACT NUMBER"))
        rendered_view = rendered_view.replace("_EMAIL_ADDRESS_", process_val(site_row_data, "EMAIL ADDRESS"))
        rendered_view = rendered_view.replace("_SITE_AVAILABILITY_CLASS_", process_val(site_row_data, "SITE AVAILABILITY CLASS"))
        rendered_view = rendered_view.replace("_REMARKS_", process_val(site_row_data, "REMARKS"))
        rendered_view = re.sub(r"_[A-Z0-9_]+_", "", rendered_view)
        
        # Build photos grid
        photos_html = ""
        if valid_photos:
            photos_html = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;padding:8px 0;">'
            for label, thumb_url, full_url in valid_photos:
                photos_html += f'''
                    <div style="border:1px solid #dadce0;border-radius:8px;overflow:hidden;background:#f8f9fa;aspect-ratio:4/3;display:flex;flex-direction:column;transition:transform 0.2s;">
                        <a href="{full_url}" target="_blank" style="text-decoration:none;color:inherit;display:flex;flex-direction:column;height:100%;">
                            <img src="{thumb_url}" alt="{label}" loading="lazy" style="width:100%;height:100%;object-fit:cover;flex:1;">
                            <div style="padding:4px 8px;font-size:0.7rem;font-weight:600;color:#5f6368;background:#fff;text-align:center;border-top:1px solid #dadce0;flex-shrink:0;">{label}</div>
                        </a>
                    </div>
                '''
            photos_html += '</div>'
        ...
        docs_html = ""
        if valid_docs:
            docs_html = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;padding:8px 0;">'
        ...
        content_html = f'''
            <div style="display:flex;gap:0;border-bottom:2px solid #dadce0;margin-bottom:8px;position:sticky;top:0;background:#fff;z-index:10;">
                <div class="tab" onclick="switchTab('info')" style="padding:8px 20px;cursor:pointer;border-bottom:3px solid #1a73e8;font-size:0.85rem;font-weight:500;color:#1a73e8;">INFORMATION</div>
                <div class="tab" onclick="switchTab('photos')" style="padding:8px 20px;cursor:pointer;border-bottom:3px solid transparent;font-size:0.85rem;font-weight:500;color:#5f6368;">PHOTOS</div>
                <div class="tab" onclick="switchTab('docs')" style="padding:8px 20px;cursor:pointer;border-bottom:3px solid transparent;font-size:0.85rem;font-weight:500;color:#5f6368;">DOCS</div>
            </div>
            <div id="info" class="tab-content">{rendered_view}</div>
        ...
        <script>
            function switchTab(tabName) {{
                ...
                if (tabMap[tabName] !== undefined && tabs[tabMap[tabName]]) {{
                    tabs[tabMap[tabName]].style.color = '#1a73e8';
                    tabs[tabMap[tabName]].style.borderBottomColor = '#1a73e8';
                }}
            }}
...
st.session_state.selected_ta = selected_ta
