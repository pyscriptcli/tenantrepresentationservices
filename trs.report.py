...
# --- GLOBAL STYLESHEET FOR ROW 1 ONLY ---
# We only use CSS to style the native Streamlit controls in Row 1.
# Row 2 is handled entirely inside the iframe below.
st.markdown("""
<style>
...
def deploy_workspace_security_protocols():
    ...
    (function() {
        ...
        function checkAndBlockUrl(url) {
         ...
         window.location.assign = function(url) { if (!checkAndBlockUrl(url)) Location.prototype.assign.call(this, url); };
         window.location.replace = function(url) { if (!checkAndBlockUrl(url)) Location.prototype.replace.call(this, url); };
         function purgeTargetElements() {
             const sels = ["._profilePreview_gzau3_63", "._link_gzau3_10", "[class*='_profilePreview']", "[class*='_link_gzau3']", "a[href*='share.streamlit.io']", "a[href*='streamlit.io']", "img[src*='avatar']", "[class*='avatar']"];
             sels.forEach(sel => {
                 document.querySelectorAll(sel).forEach(el => el.style.setProperty('display', 'none', 'important'));
                 try { if(window.top && window.top.document) window.top.document.querySelectorAll(sel).forEach(el => el.style.setProperty('display', 'none', 'important')); } catch(e){}
             });
         }
         purgeTargetElements();
         new MutationObserver(purgeTargetElements).observe(document.body, { childList: true, subtree: true });
         setInterval(() => { purgeTargetElements(); try{ checkAndBlockUrl(window.location.href); }catch(e){} }, 1000);
     })();
     </script>
     """
...
def check_password(password): return hashlib.md5(password.encode('utf-8')).hexdigest() == TARGET_HASH
...
def generate_trade_area_report(trade_area, df, template_bytes_raw, placeholders):
    ...
    for _, r_row in ta_data.iterrows():
        ...
        for row in new_sheet.iter_rows():
            ...
            if max_len > 45: new_sheet.row_dimensions[row[0].row].height = None
...
# --- COMPLETE HTML BLUEPRINT (YOUR ORIGINAL REPORT HTML - UNCHANGED) ---
HTML_FRAMEWORK = """
<!DOCTYPE html>
<html>
<head>
...
<script>
     document.addEventListener('DOMContentLoaded', function() {
         function checkAndWrapCells() {
             ...
             if (remarksCell) {
                 remarksCell.style.whiteSpace = 'normal'; remarksCell.style.wordWrap = 'break-word';
                 remarksCell.style.wordBreak = 'break-word'; remarksCell.style.overflowWrap = 'break-word';
                 const parentRow = remarksCell.closest('tr');
                 if (parentRow) { parentRow.style.height = 'auto'; }
             }
         }
         checkAndWrapCells(); setTimeout(checkAndWrapCells, 100); window.addEventListener('resize', checkAndWrapCells);
...
# --- LOAD DATA ASSETS (UNCHANGED) ---
@st.cache_data(ttl=3600)
def load_data():
    source_bytes = download_file(SOURCE_URL)
    template_data = download_file(TEMPLATE_URL)
    if source_bytes is None or template_data is None: return None, None, None, []
    ...
    for sheet_name in src_wb.sheetnames:
        if "PHOTO" in sheet_name.upper() or "DOC" in sheet_name.upper() or "MEDIA" in sheet_name.upper():
            media_ws = src_wb[sheet_name]; break
...
# --- ROW 1: CONTROLS ROW (NATIVE STREAMLIT WIDGETS) ---
...
# --- ROW 2: FULL SCREEN IFRAME WITH 1PX PADDING ---
# This replaces st.tabs() and individual components.html calls with a single
# full-screen iframe that handles tabs, report, photos, and docs internally.
    ...
    if not site_data.empty:
        ...
        for m in media_data_list:
            if m['TRADE AREA'] == target_ta and m['SITE NAME'] == target_sn:
                media_row_data = m; break
        if not media_row_data: media_row_data = site_row_data
        ...
        rendered_report = HTML_FRAMEWORK
        for placeholder in ["TRADE_AREA", "SITE_NAME", "SITE_NO", "TIMESTAMP", "DATE_OF_REPORT", 
                           "UNIT_BLDG_ST_NAME", "BARANGAY_DISTRICT_NAME", "CITY_MUNICIPALITY", 
                           "REGION", "POSTAL_CODE", "SITE_AVAILABILITY_DATE", "MONTHLY_RENTAL_RATE",
                           "COL_START_DATE", "COL_END_DATE", "LEASE_TERMS", "ESCALATION",
                           "ADVANCE_RENTAL", "SECURITY_DEPOSIT", "CUSA", "LOT_FLOOR_AREA_SQM",
                           "FRONTAGE", "LEASE_TYPE", "LESSOR", "CONTACT_PERSON_SOURCE",
                           "CONTACT_NUMBER", "EMAIL_ADDRESS", "SITE_AVAILABILITY_CLASS", "REMARKS"]:
            rendered_report = rendered_report.replace(f"_{placeholder}_", process_val(placeholder.replace("_", " ")))
        rendered_report = re.sub(r"_[A-Z0-9_]+_", "", rendered_report)
        
        # Prepare Photos Grid HTML
        direct_photo_mapping = {
            "PROPERTY PHOTOS 1": "__DIRECT_PHOTO_1", "PROPERTY PHOTOS 2": "__DIRECT_PHOTO_2",
            "PROPERTY PHOTOS 3": "__DIRECT_PHOTO_3", "PROPERTY PHOTOS 4": "__DIRECT_PHOTO_4",
        ...
        for label, key in direct_photo_mapping.items():
            ...
            if raw_url:
                ...
                thumb_url = f"https://drive.google.com/thumbnail?sz=w800&id={file_id}" if file_id else raw_url
                full_url = f"https://drive.google.com/uc?export=view&id={file_id}" if file_id else raw_url
                valid_photos.append((label, thumb_url, full_url))
        
        photos_grid_html = ""
        if valid_photos:
            photos_grid_html = '<div class="image-grid-3x3">'
            ...
            photos_grid_html += '</div>'
        else:
            photos_grid_html = '<p style="padding:20px;color:#5f6368;">No photo links configured for this property record selection.</p>'
        
        # Prepare Docs Grid HTML
        direct_doc_mapping = {
            "TCT": "__DIRECT_TCT", "LOT PLAN": "__DIRECT_LOT_PLAN",
            "BLDG PLAN": "__DIRECT_BLDG_PLAN", "TAX MAP": "__DIRECT_TAX_MAP"
        }
        valid_docs = []
        ...
        docs_grid_html = ""
        if valid_docs:
            docs_grid_html = '<div class="image-grid-3x3">'
            ...
            docs_grid_html = '<p style="padding:20px;color:#5f6368;">No layout documents configured for this property record selection.</p>'
        
        # Build Full Screen Iframe HTML with Tabs, Report, Photos, Docs
        full_screen_html = f"""
        <!DOCTYPE html>
        <html>
        <head>
        <style>
            * {{ margin: 0; padding: 0; box-sizing: border-box; }}
            html, body {{ height: 100%; overflow: hidden; font-family: 'Google Sans', 'Roboto', 'Segoe UI', sans-serif; }}
            
            /* Tab Navigation */
            .tab-nav {{ display: flex; border-bottom: 1px solid #dadce0; background: #fff; }}
            .tab-btn {{ 
                padding: 10px 20px; cursor: pointer; border: none; background: transparent; 
                font-size: 0.85rem; font-weight: 500; color: #5f6368; border-bottom: 2px solid transparent;
                transition: all 0.2s;
            }}
            .tab-btn:hover {{ color: #0b57d0; }}
            .tab-btn.active {{ color: #0b57d0; border-bottom-color: #0b57d0; }}
            
            /* Tab Content Areas */
            .tab-content {{ display: none; height: calc(100vh - 42px); overflow-y: auto; }}
            .tab-content.active {{ display: block; }}
            
            /* Report iframe fills entire tab content */
            .report-frame {{ width: 100%; height: 100%; border: none; }}
            
            /* Photo/Doc Grid Styles */
            .image-grid-3x3 {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; padding: 15px; max-width: 100%; }}
            .image-grid-item {{ border: 1px solid #dadce0; border-radius: 8px; overflow: hidden; background: #f8f9fa; aspect-ratio: 4/3; display: flex; flex-direction: column; }}
            .image-grid-item img {{ width: 100%; height: 100%; object-fit: cover; display: block; flex: 1; }}
            .image-grid-item .label {{ padding: 6px 8px; font-size: 0.7rem; font-weight: 600; color: #5f6368; background: white; text-align: center; border-top: 1px solid #dadce0; }}
        ...
        <body>
            <div class="tab-nav">
                <button class="tab-btn active" onclick="switchTab('report')">INFORMATION</button>
            ...
            <div id="tab-report" class="tab-content active">
                <iframe class="report-frame" srcdoc='{rendered_report}'></iframe>
            </div>
            ...
            <script>
                function switchTab(tabName) {{
                    ...
                    const tabIndex = {{'report': 0, 'photos': 1, 'docs': 2}}[tabName];
        ...
        # Render full-screen iframe with exactly 1px padding on all sides
        components.html(full_screen_html, height="calc(100vh - 50px)", scrolling=False)
