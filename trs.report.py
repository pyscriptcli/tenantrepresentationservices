...
with map_container:
    # Prepare data for JavaScript bridge
    sites_json = df[["SITE_NAME", "TRADE_AREA", "SITE_DISPLAY"]].to_json(orient='records')
    ...
    <body>
        ...
        <script>
            // Initialize Map
            var map = L.map('map').setView([14.6091, 121.0223], 15); // Default to EDSA/Mandaluyong area
            ...
            taSelect.addEventListener('change', function() {{
                ...
                if (selectedTA) {{
                    var filteredSites = sitesData.filter(s => s.TRADE_AREA === selectedTA);
                    filteredSites.sort((a,b) => parseInt(a.SITE_DISPLAY) - parseInt(b.SITE_DISPLAY));
                    
                    filteredSites.forEach(site => {{
                        var opt = document.createElement('option');
                        opt.value = site.SITE_NAME;
            ...
            function updateDropdowns(siteName) {{
                ...
                var siteObj = sitesData.find(s => s.SITE_NAME === siteName);
                if (siteObj) {{
                    taSelect.value = siteObj.TRADE_AREA;
...
# Note: In a real deployment, you'd use st.experimental_get_query_params or 
# a more robust websocket approach. For this prototype, we use a simple 
# callback mechanism via session state updates triggered by the JS postMessage.

# Since Streamlit doesn't natively support bidirectional JS communication easily 
# without custom components, we'll simulate the selection logic here.
# In production, replace this with a proper Streamlit Custom Component.

# For now, let's provide the export functionality based on current selections
...
# --- Helper to generate photo/doc grids for modal injection ---
def generate_photo_grid(media_row_data):
    if not media_row_data: return "<p>No photos available.</p>"
...
def generate_doc_grid(media_row_data):
    if not media_row_data: return "<p>No documents available.</p>"
    direct_doc_mapping = {
        "TCT": "__DIRECT_TCT",
...
# NOTE: To make the modal fully dynamic, you would need to implement a 
# Streamlit Custom Component that handles the postMessage events and 
# updates session_state, triggering a rerun to inject the HTML into the modal.
# The above HTML/JS provides the complete UI framework for this architecture.
